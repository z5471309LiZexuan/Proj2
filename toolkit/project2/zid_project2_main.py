...
'''dict_ret = etl.aj_ret_dict(['aapl', 'msft'], '2010-01-01', '2011-06-09')
# print(dict_ret)
df_cha = cha.cha_main(dict_ret, 'vol',  ['Daily',])
print(df_cha)
df_portfolios = pf.pf_main(df_cha, 'vol', 2)
print(df_portfolios)'''
...
'''x = get_avg(DM_Ret_dict['Daily'], 2008)
...
#     Use the output dictionary, DM_Ret_dict, and auxiliary function in this script
#     to do the calculation.
'''x = get_avg(DM_Ret_dict['Monthly'], 2019)
x.sort_values(inplace=True)
print(x)'''
Q3_ANSWER = 'AAPL'
...
def t_stat(sample, test_average):
    """
    sample: the dataframe that you want to test by two_side
    test_average: t-stat average, H0: sample is equal to test_average, Ha: sample is not equal to test_average
    Returns
    -------
    ls_bar: mean of sample
    ls_t: t-stat
