...
def vol_cal(ret, cha_name, ret_freq_use: list):
    ...
    for r in ret_freq_use:
        if r == 'Daily':
            ...
            verify_shit = shit.groupby(shit.index.to_period('M')).count()
            kill_shit = []
            for i_shit in range(verify_shit.shape[0]):
                all_nan_in_shit = True
                for j_shit in range(verify_shit.shape[1]):

                    if verify_shit.iloc[i_shit, j_shit] < 18:
                        period_shit.iloc[i_shit, j_shit] = np.nan
                    else:
                        all_nan_in_shit = False
                if all_nan_in_shit:
                    kill_shit.append(i_shit)
            period_shit.drop(period_shit.index[kill_shit], inplace=True)
            period_shit.columns = [holyShit + '_' + cha_name for holyShit in period_shit.columns]
            # period_shit.to_csv('1.csv')
            return period_shit
        else:
            shit = ret[r].copy()
            # print(shit)
            # print(shit.index)
            # shit = shit.set_index(pd.DatetimeIndex(pd.to_datetime(shit.index)))
            period_shit = shit.groupby(shit.index.year).std()
            # print(period_shit)
            '''verify_shit = shit.groupby(shit.index.to_period('Y')).count()
            kill_shit = []
            for i_shit in range(verify_shit.shape[0]):
                all_nan_in_shit = True
                for j_shit in range(verify_shit.shape[1]):
...
# ret_dict = etl._test_aj_ret_dict(tickers=['AAPL', 'TSLA'], start='2018-01-31', end='2018-07-24')
# vol_cal(ret_dict, 'vol', ['Daily',])
...
def merge_tables(ret, df_cha, cha_name):
    ...
    """
    # <COMPLETE THIS PART>
    ret_monthly = ret['Monthly'].copy()
    df_cha = df_cha.copy().shift()
    shit = pd.concat([ret_monthly, df_cha], axis=1)
    shit.columns = [s for s in shit.columns[:int(len(shit.columns) / 2)]] + [s + '_' + cha_name for s in shit.columns[:int(len(shit.columns) / 2)]]
    shit.index.name = 'Year_Month'
    return shit
...
if __name__ == "__main__":
    ...
    # ret_dict = etl._test_aj_ret_dict(tickers=['nVdA', 'tsla'], start='2015-12-31', end='2019-08-31')
