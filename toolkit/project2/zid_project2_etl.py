...
def monthly_return_cal(prc):
    ...
    try:
        ...
        # print(verify_shit)
        holy_shit = shit.resample('M').agg({'Adj Close': 'last'})
    except KeyError:
        verify_shit = shit.resample('M').agg({prc.columns[0]: 'count'})
        holy_shit = shit.resample('M').agg({prc.columns[0]: 'last'})
    period_shit = shit.groupby(shit.index.to_period('M')).mean()   # create a shit to replace
    '''print(holy_shit)
    print(period_shit)
    print(verify_shit)'''
    kill_shit = []
    for v in range(verify_shit.shape[0]):
        period_shit.iloc[v, 0] = holy_shit.iloc[v, 0]
        if verify_shit.iloc[v, 0] < 18:
            kill_shit.append(v)
    # print(kill_shit)
    raw_shit = period_shit / period_shit.shift() - 1
    raw_shit.drop(index=period_shit.index[kill_shit], inplace=True)
    raw_shit.dropna(inplace=True)
    # print(raw_shit)
    return raw_shit
# print(monthly_return_cal(read_prc_csv('aapl', '2018-01-01', '2018-07-13')))
...
def aj_ret_dict(tickers, start, end):
    ...
    try:
        ...
        raise Exception("you didn't enter a real datetime. try again")
    d = [daily_return_cal(read_prc_csv(t.lower(), start, end)) for t in tickers]
    m = [monthly_return_cal(read_prc_csv(t.lower(), start, end)) for t in tickers]
    # print(d)
    # print(m)
    shit_daily = pd.concat(d, axis=1)
    shit_monthly = pd.concat(m, axis=1)
    shit_daily.columns = [t.lower() for t in tickers]
    shit_monthly.columns = [t.lower() for t in tickers]
    shit_monthly.index.name = 'Year_Month'
    return {'Daily': shit_daily, 'Monthly': shit_monthly}
# print(aj_ret_dict(['AAPL', 'BABA'], '2019-01-02', '2020-01-02'))
...
def _test_read_prc_csv():
    ...
    """
    tic = 'AAPL'
    ser = read_prc_csv(tic, '2010-01-04', '2011-01-02')
    util.test_print(ser)
